...
public class QualificationController {
  ...
  private void initialize() {
    // exercice 7 (étape 1) : alimenter la TableView avec les séquences de la nuit, et
    ...
    //  exercice 7 (étape 2) : afficher dans labelSelection la séquence sélectionnée.
    ...
    tableView
        ...
        .addListener(
            (e, o, n) -> {
              if (n == null) labelSelection.setText("(sélectionnez une séquence dans le tableau)");
              else {
                labelSelection.setText(
                    String.format(
                        "Séquence %s - %.1f kHz", n.getHorodatage(), n.getFrequenceDominanteKHz()));
              }
            });
    labelSelection.setText("(sélectionnez une séquence dans le tableau)");

    // exercice 7 (étape 3) : le bouton "Écouter" est désactivé tant qu'aucune séquence
    ...
    //  exercice 7 (étape 4) : peupler la ChoiceBox avec les trois verdicts possibles :
    // "OK", "Douteux", "À jeter".
    choiceBoxVerdict.setItems(FXCollections.observableArrayList("OK", "Douteux", "À jeter"));

    // exercice 7 (étape 5) : labelVerdictGlobal doit refléter le verdict du modèle.
    ...
    //  exercice 7 (étape 6) : lier la TextArea de commentaire au modèle (binding
    // bidirectionnel).
  ...
  private void ecouter() {
    // exercice 7 (étape 7) : mettre le statut de la séquence sélectionnée à "Écoutée" et
  ...
  private void enregistrerVerdict() {
    // exercice 7 (étape 8) : lire choiceBoxVerdict.getValue() et l'enregistrer dans le
