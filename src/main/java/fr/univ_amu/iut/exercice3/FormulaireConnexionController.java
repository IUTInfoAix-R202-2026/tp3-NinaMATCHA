...
public class FormulaireConnexionController {
  ...
  private void initialize() {
    // exercice 3 : installer les bindings de validation.
    //
    ...
    BooleanBinding mdp =
        new BooleanBinding() {
          ...
          public boolean computeValue() {
            String champ = champMotDePasse.textProperty().get();
            return champ.chars().noneMatch(Character::isUpperCase)
                || champ.chars().noneMatch(Character::isDigit)
                || champ.length() < 8;
          }
        };
    boutonOk.disableProperty().bind(mdp);
  }
  ...
  private void valider() {
    // exercice 3 : afficher dans labelMessage l'identifiant suivi du mot
  ...
  private void annuler() {
    // exercice 3 : vider les deux champs et le label message.
